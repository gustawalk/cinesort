...
async function update_rating() {
  const [filmes] = await pool.query(
    'SELECT imdb_id FROM filmes WHERE imdb_rate IS NULL'
  );

  for (const filme of filmes) {
    const filme_id = filme.imdb_id;
    console.log(`Tentando pegar ${filme_id}`);

    const url = `https://www.imdb.com/title/${filme_id}/`;
    ...
    try {
      ...
      console.log(`Nota: ${imdb_rate}`);

      await pool.query('UPDATE filmes SET imdb_rate = ? WHERE imdb_id = ?', [imdb_rate, filme_id]);

    } catch (err) {
      console.error(`Erro ao pegar dados de ${filme_id}:`, err.message);
    }
...
async function get_info_from_imdb(id_filme) {
  try {
    ...
    let sinopse = $('[data-testid="plot-l"]').text() || $('[data-testid="plot-xs_to_m"]') || $('[data-testid="plot-xl"]') || "Error fetching synopsis";
